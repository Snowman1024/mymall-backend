...
public interface ProductRepository extends BaseRepository<ProductEntity, Integer> {
    ...
    @Query(nativeQuery = true,value = "SELECT p.id,p.goods_id,p.goods_specification_ids,p.goods_sn," +
            " p.goods_number,p.retail_price,p.market_price,g.name,g.list_pic_url " +
            " FROM product p LEFT JOIN goods g ON p.goods_id = g.id " +
            " WHERE p.id = :id ")
    List<Object> queryById(@Param(value = "id")Integer id);
}
